...
def main(basis_number = 100, CV_candidates = [.1, .5, .7, .9, .95, .99, 1], CV_folds = 10):
	...
	ElasticNet_regression = linear_model.ElasticNetCV(l1_ratio =  CV_candidates, max_iter = 10000, cv = cross_validation.KFold(sample_number, n_folds = CV_folds))
...
if __name__ == '__main__': 
	...
	p = Process(target = main, args = ( 500, [.1, .5, .7, .9, .95, 1]))
	p.start()
	p.join()