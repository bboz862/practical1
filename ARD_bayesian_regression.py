...
def main(basis_number = 500):
	...
	#do ridge regression
	ARD_regression = linear_model.ARDRegression( n_iter = 5000)
	ARD_regression.fit(transformed_molecular_features, molecule_energygap)
	
	# print ARD_regression.coef_

	#calculate prediction accuracy on training data
	predicted_train_gap_energy = ARD_regression.predict(transformed_molecular_features)
	...
	predicted_test_gapenergy = ARD_regression.predict(transformed_test_features)
