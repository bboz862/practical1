...
def main(basis_number = 100, CV_candidates = None, CV_folds = 10):
	...
	read_molecule_data = read_compact_data('meaningful_features_600000.csv')
	...
	lasso_regression = linear_model.LassoCV(alphas =  CV_candidates, max_iter = 10000, cv = cross_validation.KFold(sample_number, n_folds = CV_folds))
	...
	pridected_train_gap_energy = lasso_regression.predict(transformed_molecular_features)
	train_average_error = math.sqrt( np.dot( (pridected_train_gap_energy - molecule_energygap), (pridected_train_gap_energy - molecule_energygap).T ) / float(sample_number) )
	print train_average_error

if __name__ == '__main__': main(basis_number = 200, CV_candidates = [ 0.000001, 0.000005, 0.00001 ])